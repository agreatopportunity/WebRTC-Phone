...
app.post('/api/messages/send', async (req, res) => {
  ...
  try {
    ...
    const [result] = await db.query(
      ...
      [conversation_id, 'visitor', visitorDbId, message]
    );
...
app.get('/api/messages/:visitorId', async (req, res) => {
  ...
  try {
    const [messages] = await db.query(
      `SELECT m.id, m.sender_type as senderType, m.content, m.created_at as createdAt,
...
app.post('/api/messages/upload', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    const messageContent = req.file.mimetype.startsWith('image/') 
      ? '📷 Image' 
      : `📎 ${req.file.originalname}`;
    
    const [messageResult] = await db.query(
      'INSERT INTO messages (conversation_id, sender_type, sender_id, content) VALUES (?, ?, ?, ?)',
      [conversation_id, 'visitor', visitorDbId, messageContent]
    );
...
app.get('/api/owner/conversations/:id/messages', requireAuth, async (req, res) => {
  ...
  try {
    const [messages] = await db.query(
      `SELECT m.id, m.sender_type as senderType, m.content, m.is_read as isRead, m.created_at as createdAt,
              a.filename, a.original_name as originalName, a.mime_type as mimeType
       FROM messages m
       LEFT JOIN attachments a ON a.message_id = m.id
...
app.post('/api/owner/messages/upload', requireAuth, upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    const [messageResult] = await db.query(
      'INSERT INTO messages (conversation_id, sender_type, sender_id, content, is_read) VALUES (?, ?, ?, ?, TRUE)',
      [conversationId, 'owner', req.session.ownerId, messageContent]
    );
